...
class HandArmController:
    ...
    执行器命名约定：
        ...
        灵巧手:
            - "hand_act_push_{0-3}" → 四指弯曲 (finger_0-3)
            - "hand_thumb_grasp"    → 拇指弯曲
            - "hand_thumb_rotate"   → 拇指旋转
...
def example_usage():
    """
    HandArmController 的使用示例（不作为主程序运行）.

    展示初始化、查询限制、应用控制的完整流程。
    """
    import mujoco
    
    # 1. 加载模型
    model = mujoco.MjModel.from_xml_path("robot_arm_hand.xml")
    ...
    # 3. 查看执行器信息
    print("机械臂执行器:", controller.arm_names)
    print("灵巧手执行器:", controller.hand_names)
    print("机械臂力矩限制:", controller.get_arm_torque_limits())
    
    # 4. 仿真循环中的控制
    for _ in range(1000):
        # 获取当前状态（用于计算控制律）
        q_arm = data.qpos[controller.arm_indices]
        dq_arm = data.qvel[controller.arm_indices]
        
        # 简单的 PD 控制（示例）
        q_target = np.zeros(7)
        tau_arm = 100.0 * (q_target - q_arm) - 10.0 * dq_arm
        
        # 恒定抓握力
        tau_hand = np.array([0.5, 0.5, 0.5, 0.5, 1.0, 0.3])
        
        # 应用控制（自动处理单位转换和限幅）
        controller.apply_control(data, tau_arm, tau_hand)
