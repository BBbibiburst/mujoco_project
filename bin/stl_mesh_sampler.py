...
def _detect_arc_range(angles_deg):
    ...
    for i, e in enumerate(double):
        if e:
            if not in_g:
                c_start, c_len, in_g = i, 1, True
            else:
                c_len += 1
            if c_len > b_len:
                b_len, b_start = c_len, c_start
        else:
            in_g = False
...
def generate_surface_mesh_points_from_stl(
    stl_path, m, n, ransac_iters=100, inlier_tol=0.15, use_cache=True
):
    """
    ...
    - inlier_tol:   RANSAC 内点容差比例。
    - use_cache:    是否启用磁盘缓存，默认 True。
