...
class OSC_PositionController:
    ...
    数学原理：
        任务空间动力学：Λ = (J M⁻¹ Jᵀ)⁻¹ （操作空间惯量矩阵）
        任务空间力：F = Λ (ẍ_des - J̇q̇) + μ + p
        关节力矩：τ = Jᵀ F + Nᵀ τ_null + (I - JᵀJ̄)ᵀ bias
    ...
    def _update_velocity_feedforward(
        ...
        计算目标速度：
            v_raw = (target_curr - target_prev) / dt
        
        滤波更新：
            v_ff = alpha * v_raw + (1 - alpha) * v_ff_prev
    ...
    def _compute_task_acceleration(
        ...
        位置控制：
            e_pos = target - current
            e_vel = ff_scale * v_ff - J_p @ qd
            xacc_des = kp_pos * e_pos + kd_pos * e_vel

        姿态控制（可选）：
            处理四元数符号二义性（确保最短路径旋转）
            e_rot = quat2Vel(target * conj(current))
            e_vel_rot = ff_scale * w_ff - J_r @ qd
            xacc_rot = kp_rot * e_rot + kd_rot * e_vel_rot
        ...
        Returns:
            Tuple[np.ndarray, np.ndarray]: (xacc_des, xacc_rot) 期望线加速度和角加速度。
        """
        # 位置误差
    ...
    def _compute_osc_torque(
        ...
        零空间补偿：
            ...
            tau_bias_null = Nᵀ @ bias  （仅补偿零空间方向的 bias）
...
class IK_PositionController:
    ...
    该控制器采用两层架构：
        1. 上层 (IK Solver): 将末端执行器的空间目标转换为关节空间的目标增量。
           使用基于雅可比的数值方法（自适应 DLS）。
        2. 下层 (PD Controller): 接收关节目标，计算所需的关节力矩并应用饱和限制。

    适用场景：初始化、粗略轨迹预定位或传统关节空间控制。
    对于精确轨迹跟踪，建议使用 OSC_PositionController。

    Attributes:
        base: 硬件/模型抽象接口。
        model: MuJoCo 模型对象。
        gains: PD 增益配置。
        arm_qpos_ids/arm_qvel_ids/arm_joint_ids: 机械臂索引。
        hand_qpos_ids/hand_qvel_ids/hand_joint_ids: 手部索引。
        arm_range/hand_range: 关节限位。
        _torque_min/_torque_max: 力矩限制。
        _arm_torques/_hand_torques: 力矩缓冲区。
        ee_site_name/ee_id: 末端 Site 名称和 ID。
        jac_p/jac_r: 雅可比缓冲区。
    """

    def __init__(self, base, model, gains: Optional[PDGains] = None):
        """
        初始化控制器.
        ...
        Args:
            base: 包含机器人硬件参数的基础对象。
            model: MuJoCo 模型实例。
            gains: 可选的自定义 PD 增益，None 时使用默认值。
        """
        self.base = base
        self.model = model
    ...
    def set_ee_target(
        ...
        if ee_quat_target is not None:
            ...
            # 计算四元数误差：error_quat = target * conj(current)
