...
使用方法：
    1. 在控制循环中创建 HandArmController 实例，传入已编译的 MuJoCo 模型对象。
    2. 使用 apply_control 或 apply_control_vector 方法，将计算出的力矩控制信号应用到仿真数据对象。
    3. 可选地，使用 get_arm_torque_limits 和 get_hand_torque_limits 方法获取当前模型的物理力矩限制，以便在控制律中进行饱和处理。
"""
import mujoco
import numpy as np
...
class HandArmController:
    ...
    执行器命名约定（关键词匹配模式）：
    - 机械臂: 包含 "torq_joint" 关键词 (如 torq_joint0, robot_arm_torq_joint1)
    - 灵巧手: 包含特定功能关键词
        * 包含 "hand_act_push_0" → finger_0 
        * 包含 "hand_act_push_1" → finger_1 
        * 包含 "hand_act_push_2" → finger_2 
        * 包含 "hand_act_push_3" → finger_3 
        * 包含 "hand_thumb_grasp" → thumb_grasp (拇指弯曲)
        * 包含 "hand_thumb_rotate" → thumb_rotate (拇指旋转)

    物理约束说明：
    MuJoCo 中 actuator.ctrl 是控制信号，实际输出力矩为：torque = ctrl * gear
    ...
    def __init__(self, model: mujoco.MjModel):
        ...
        Args:
            model: MuJoCo 模型对象，需包含完整的执行器定义。
                   要求模型中执行器命名包含约定的关键词（如 torq_joint, hand_act_push）。
    ...
    def _build_map(self):
        """
        遍历模型中的所有执行器，根据通用关键词规则构建映射表。

        命名约定解析（基于关键词匹配，不再依赖特定前缀）：
        - 机械臂: 包含 "torq_joint" 字符串，后跟数字索引
        - 灵巧手: 包含特定功能关键词，映射到逻辑名称
            * 包含 "hand_act_push_0" → finger_0
            * 包含 "hand_act_push_1" → finger_1
            * 包含 "hand_act_push_2" → finger_2
            * 包含 "hand_act_push_3" → finger_3
        ...
        # 例如：...torq_joint0, ...torq_joint1, ...torq_joint6
        # 使用正则提取数字进行排序，兼容任意前缀
        self.arm_names.sort(key=lambda x: int(''.join(filter(str.isdigit, x))))
    ...
    def _build_index_arrays(self):
        ...
        try:
            self.hand_indices = np.array(
                [self.actuator_map[self.hand_names[k]] for k in self.hand_key_order],
                dtype=np.int32,
            )
        except KeyError as e:
            raise KeyError(f"未找到匹配的执行器关键词，请检查模型命名: {e}")
    ...
    def _to_ctrl_signal(self, torques: np.ndarray, indices: np.ndarray) -> np.ndarray:
        ...
        """
        # 第一重限幅：物理力矩限制（保护硬件）
        torque_clipped = np.clip(
            torques, self.torque_min[indices], self.torque_max[indices]
        )
    ...
    def apply_control(
        self, data, arm_torques: np.ndarray, hand_torques: np.ndarray
    ):
        """
        分别应用机械臂和灵巧手的力矩控制。
        ...
        Args:
            ...
            hand_torques: 灵巧手关节力矩 [N·m]，shape (6,)。
                          顺序为 [finger_0, finger_1, finger_2, finger_3, 
        ...
        if arm_torques.shape != (self.ARM_DOF,):
            raise ValueError(
                f"Arm torque shape mismatch: expected {(self.ARM_DOF,)}, "
                f"got {arm_torques.shape}"
            )
        if hand_torques.shape != (self.HAND_DOF,):
            raise ValueError(
                f"Hand torque shape mismatch: expected {(self.HAND_DOF,)}, "
                f"got {hand_torques.shape}"
            )
    ...
    def apply_control_vector(self, data, control_vector: np.ndarray):
        ...
        data.ctrl[self.all_indices] = self._to_ctrl_signal(
            control_vector, self.all_indices
        )
