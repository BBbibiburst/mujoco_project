...
该模块实现从STL网格自动分离并拟合厚壁椭圆柱（灵巧手所用的皮肤stl）内外表面，生成外表面规则矩阵采样点云，以方便后续触觉传感器布置和仿真。
采用基于RANSAC的鲁棒椭圆拟合、法向量辅助的内外表面分离、以及弧长均匀的网格采样策略。
这个代码只展示可视化，用来进行算法开发和结果验证。核心计算模块在stl_mesh_sampler.py中实现，去除可视化依赖，更适合集成和性能优化。
...
使用方法：
    # 直接指定STL文件路径
    python -m source.sensors.stl_mesh_sampler_with_plotter --stl_path /path/to/your/model.stl
    # 示例：使用项目内置的STL文件
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="Thick-wall Elliptic Cylinder Fitting")
