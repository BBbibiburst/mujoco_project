...
def _make_task_cfg(task_name: str, overrides: Optional[Dict[str, Any]] = None) -> Optional[Any]:
    """
    构造任务配置，支持通过命令行覆盖物体描述符等参数。
    ...
    task_name : str
        任务名称
    overrides : dict | None
        需要覆盖的配置项，如 {"obj_descriptor": "sphere_blue_small"}

    返回
    ----
    任务配置实例，或 None（如果该任务不需要额外配置或没有覆盖）
    """
    if not overrides:
        return None

    # 延迟导入，避免循环依赖
    if task_name == "block_lifting":
        from source.env.block_lifting_env import BlockLiftingConfig
        return BlockLiftingConfig(**overrides)
...
def _pipeline_worker_stream(args: dict, result_q: _mp.Queue) -> None:
    ...
    log_info        = args["log_info"]
    worker_id       = args["worker_id"]
    task_config_overrides = args.get("task_config_overrides", None)  # <-- 新增
    ...
    task_cfg = _make_task_cfg(task_name, task_config_overrides)  # <-- 新增
    env = load_task(task_name, robot_cfg, task_config=task_cfg)  # <-- 修改
    strategy = load_strategy(task_name)
        ...
        worker_args = [
            {
                ...
                "worker_id": i,
                "log_dir": log_dir_str,
                "task_config_overrides": task_config_overrides,  # <-- 新增
            }
            for i, chunk in enumerate(chunks)
            if chunk  # 过滤空分片（n_workers > n_episodes 时出现）
        ]
