...
def demo_sinusoid(
    task_name: str = "pick_place",
    freq: float = 0.5,        # 正弦波频率 (Hz)
    amplitude: float = 0.5,   # 正弦波幅度 (弧度)
    n_episodes: int = 1,
    render: bool = True,
):
    """
    ...
    # 必须使用 joint_pd 才能直接控制关节角度
    robot_cfg = RobotConfig(
        action_mode="joint_pd", 
        controller_type="ik",   
        max_episode_steps=1000, # sinusoid 模式建议增加步数以便观察
        control_freq=20.0,
        tactile_backend="simple_avg",
    )
    ...
    if render:
        ...
        with mujoco.viewer.launch_passive(env.model, env.data) as viewer:
            ...
            while viewer.is_running():
                ...
                # 3. 更新 MuJoCo Viewer 画面
                viewer.sync()

                # 4. OpenCV 辅助可视化 (触觉+相机)
                heatmap = render_tactile_heatmap(obs)
                cv2.imshow("Tactile (Top/Mid/Bot)", heatmap)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode", 
        choices=["random", "verify", "benchmark", "sinusoid"], # <--- 这里加了 "sinusoid"
        default="random", 
        help="演示模式",
    )
